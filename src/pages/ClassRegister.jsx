...
const ClassRegister = () => {
  ...
  const loadAttendanceForDate = async (classId, date, classStudents) => {
    ...
    try {
      ...
      if (attendanceData.success && attendanceData.students) {
        ...
        console.log('📝 No existing attendance found, initializing with default values');
  ...
  const handleClassChange = (newClassId) => {
    const newClass = teacherClasses.find(c => c.id === parseInt(newClassId));
    if (newClass) {
      setSelectedClassId(newClass.id);
      setTeacherClass(newClass);
      setClassName(newClass.name);
      // Will trigger useEffect to reload students and attendance
    }
  };
